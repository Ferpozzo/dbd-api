...
async function handleSuggest(req, res) {
  try {
    ...
    const type = req.query.type ? req.query.type.charAt(0).toUpperCase() + req.query.type.slice(1).toLowerCase() : null;
    if (!['Survivor', 'Killer'].includes(type))
      return res.status(400).send(Prettify._JSON({ error: 'Invalid or missing type parameter' }));

    // 🔹 Filtra perks por keywords
    ...
    const results = [];
    const topN = 10; // limitar vizinhos
      ...
      for (let i = 0; i < neighborsA.length; i++) {
        ...
        for (let j = 0; j < neighborsB.length; j++) {
          ...
          for (let k = 0; k < neighborsC.length; k++) {
            ...
            results.push({ combo, similaritySum });
          }
        }
      }
    }

    results.sort((a,b)=>b.similaritySum-a.similaritySum);
    const top3 = results.slice(0,3).map(r => r.combo.map(p => ({ _id: p._id, name: p.name, categories: p.categories })));
